...
def getWflMemo(templateValues):
    return """
    <process command="./wsh.php --api=importDocuments --file=./@APPNAME@/%s__WFL.csv"/>"""%(templateValues['familyName'])

def generateWorkflow(templateValues, args):
    targetsPath ={
        'wflCsv': os.path.join(args.targetDir, "%s__WFL.csv"%(args.familyName)),
        'wflPhp': os.path.join(args.targetDir, "%s__WFL__CLASS.php"%(args.familyName))
    }
...
def main():
    ...
    templateValues['workflowClass'] = "%s_WFL"%(templateValues['familyName'])
