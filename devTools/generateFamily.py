...
def parseOptions():
    ...
    if(not args.familyTitle):
        args.familyTitle = "%s"%(args.familyName.upper())
    return args

def getStructMemo(templateValues):
    return """<process command="./wsh.php --api=importDocuments --file=./%s/%s__STRUCT.csv"/>"""%(templateValues['targetDir'], templateValues['familyName'])

def getParamMemo(templateValues):
    return """<process command="./wsh.php --api=importDocuments --file=./%s/%s__PARAM.csv"/>"""%(templateValues['targetDir'], templateValues['familyName'])

def generateFamily(templateValues, args):
    targetsPath ={
        'csvStruct': os.path.join(args.targetDir, "%s__STRUCT.csv"%(args.familyName)),
        'csvParam' : os.path.join(args.targetDir, "%s__PARAM.csv"%(args.familyName)),
        'class': os.path.join(args.targetDir, "%s__CLASS.php"%(args.familyName))
    }
...
def main():
    ...
    templateValues = {
        ...
        'familyIcon'    : "%s.png"%(args.familyName),
        'familyMethod'  : "Method.%s.php"%(args.familyName),
        'familyDFLID'   : "FLD_%s"%(args.familyName.upper()),
        'familyName'    : args.familyName,
        'familyClass'   : args.familyName,
